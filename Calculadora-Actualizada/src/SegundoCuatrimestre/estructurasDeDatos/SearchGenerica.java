package SegundoCuatrimestre.estructurasDeDatos;

import java.util.ArrayList;
import java.util.Comparator;

//busquedas genericas lineales y binarias
public class SearchGenerica<T extends Comparable<T> > {
        public int lineal(ArrayList<T> a, T b, T b1 , boolean d){
            int pos = -1;
            int iter = 0;
            // compareTo
            ...
            for (int i = 0; i < a.size(); i++){
                if (b.compareTo(a.get(i)) == 0 || b1.compareTo(a.get(i)) == 0) {
                    pos = i;
                }
                if (d){
                    iter++;
